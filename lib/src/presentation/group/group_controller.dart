...
class GroupController {
  ...
  /// configurations.
  GroupController(this._engine, this._aodvManager, this._datalinkManager,
      this._eventStream, Config config) {
...
/*-------------------------------Public methods-------------------------------*/
  ...
  void createGroup([List<String>? members]) {
    ...
    var low = 8;
    var seed = max(low + low, Random(42).nextInt(32));
    ...
    var info =
        GroupInit(timestamp, _p.toString(), _g.toString(), _ownLabel, false);
...
/*------------------------------Private methods-------------------------------*/
  ...
  void _computeGroupKey(int type, [String? label]) async {
    ...
    switch (type) {
      ...
      case GROUP_JOIN:
        if (_groupKeyHash == null) {
          _groupKeyHash = await _computeGroupKeyHash();
        }

        if (label == null) {
          groupKeySum = _groupKeyHash! ^ _k!;
        } else {
          groupKeySum =
    ...
    if (length < keyLengthRequired) {
      keyBytes = Uint8List.fromList(keyBytes.toList().sublist(0, length) +
          List.filled(keyLengthRequired - length, 42));
  ...
  void _processDataReceived(AdHocDevice sender, SecureData secureData) async {
    ...
    switch (secureData.type) {
      case GROUP_INIT:
        ...
        var data =
            GroupInit.fromJson(secureData.payload as Map<String, dynamic>);
      ...
      case GROUP_LIST:
        ...
        var data =
            GroupList.fromJson(secureData.payload as Map<String, dynamic>);
      ...
      case GROUP_SHARE:
        ...
        var data =
            GroupValue.fromJson(secureData.payload as Map<String, dynamic>);
        ...
        if (_DHShare.length == _memberLabel.length) {
          for (var label in _memberLabel) {
            ...
            var msg =
                SecureData(GROUP_KEY, GroupValue(crtij.toString()).toJson());
      ...
      case GROUP_KEY:
        ...
        _CRTShare.putIfAbsent(senderLabel, () {
          _recovered += 1;
          return crtji;
        });
      ...
      case GROUP_JOIN_REQ:
        ...
        var response = GroupJoin(
            hash: groupKeyHash.toString(),
            labels: labels,
            values: values,
            share: _p.toString(),
            solution: _g.toString());
      ...
      case GROUP_JOIN_REP:
        // Retrieve joining member label data
        var data =
            GroupJoin.fromJson(secureData.payload as Map<String, dynamic>);

        // New member proceeds with the protocol from Step 3.
        if (data.hash != 'null') {
          _groupOwner = senderLabel;

          _p = BigInt.parse(data.share!);
          _g = BigInt.parse(data.solution!);

          // Hash of group key
          _groupKeyHash = BigInt.parse(data.hash!);
          ...
          // Old member updating the group key
          _memberLabel.add(senderLabel);
      ...
      case GROUP_LEAVE:
        // Retrieve joining member label data
        var data =
            GroupLeave.fromJson(secureData.payload as Map<String, dynamic>);
        ...
        if (_groupOwner == _ownLabel) {
          ...
          for (var label in _memberLabel) {
            if (label != _ownLabel) {
              ...
              var response =
                  GroupLeave(data.leavingLabel, newSolution: crtij.toString());
      ...
      case GROUP_DATA:
        ...
        var decrypted = await _engine.decrypt(
            secureData.payload as List<dynamic>,
            sharedKey: _groupKey!);
